...
export async function initManagerAndNavigate(url: string, bookId: string, getPlayList: Function) {
  const needInit = bookId !== sentencePlayManager.bookId || sentencePlayManager.sentenceList.length === 0
  if (needInit) {
    console.log('重新获取学习列表')
    wx.showLoading({ title: '获取学习列表。', mask: true })
    try {
      const pl = await getPlayList(bookId, 20)
      console.log(pl)
      sentencePlayManager.init(bookId, pl, (cursor, limit) => getPlayList(bookId, limit, cursor))
    } finally {
      wx.hideLoading()
    }
  }
